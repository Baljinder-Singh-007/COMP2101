...
#eth=ip a |awk '/: e/{gsub(/:/,"");print $2}'
#IP=route -n | grep '^0.0.0.0' | awk '{print $2}'
honame=$(hostname)
int_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
IP1=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
lanadr=$(ip a s $int_name |awk '/inet /{gsub(/\/.*/,"");print $2}')
hname=$(getent hosts $IP1 | awk '{print $2}')
extIP=$(curl -s icanhazip.com)
extname=$(getent hosts $extIP | awk '{print $2}')
routeraddr=$(route -n | grep '^0.0.0.0' | awk '{print $2}')
routername=$(getent hosts $routeraddr | awk '{print $2}')
cat <<EOF
Hostname        : $honame
LAN Address     : $lanadr
LAN Hostname    : $hname
